  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        if (sm['openai.api_key']) {
          setApiKey(sm['openai.api_key']);
        }
      } catch (err) {
    ...
    >
      ...
      <div>
        ...
        <input
          ...
          type="password"
          autoComplete="off"
          placeholder={isSet ? '--------  (leave blank to keep current)' : 'sk-proj-xxxxxxxxxxxxxxxxxxxxxxxxx'}
          value={isSet ? '' : apiKey}
        ...
        <p className="mt-1 text-xs text-neutral-500">
          ...
          . Used for AI-powered route experience discovery. Set the{' '}
