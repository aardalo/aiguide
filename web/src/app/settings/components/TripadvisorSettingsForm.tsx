...
export default function TripadvisorSettingsForm() {
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        if (sm['tripadvisor.api_key']) {
          setApiKey(sm['tripadvisor.api_key']);
        }
      } catch (err) {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const res = await fetch('/api/settings', {
        ...
        body: JSON.stringify({ key: 'tripadvisor.api_key', value: apiKey }),
      });
      if (!res.ok) {
        const data = await res.json();
        throw new Error(data.error ?? 'Failed to save');
      }
      setSuccessMessage('Tripadvisor API key saved.');
      setApiKey('[SET]');
    } catch (err) {
  ...
  return (
    ...
    >
      ...
      <div>
        <label htmlFor="tripadvisorKey" className="block text-sm font-medium text-neutral-700 mb-1">
          API Key
          {isSet && (
        ...
        <input
          ...
          type="password"
          autoComplete="off"
          placeholder={isSet ? '--------  (leave blank to keep current)' : 'xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx'}
          value={isSet ? '' : apiKey}
        ...
        <p className="mt-1 text-xs text-neutral-500">
          Get a key at{' '}
          <a
            href="https://www.tripadvisor.com/developers"
            target="_blank"
          ...
          . Set the{' '}
          <code className="bg-neutral-100 px-1 rounded text-neutral-700">TRIPADVISOR_API_KEY</code>
