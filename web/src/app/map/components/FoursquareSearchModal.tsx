...
const CATEGORIES = [
  { value: 'food',      label: 'Food & Drink', icon: 'F' },
  { value: 'hotels',    label: 'Hotels',       icon: 'H' },
  { value: 'outdoors',  label: 'Outdoors',     icon: 'O' },
...
export default function FoursquareSearchModal({ onSelect, onClose }: Props) {
  useEffect(() => {
    ...
    >
      ...
      >
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-base font-semibold text-neutral-900">Search Foursquare</h2>
          <button
            onClick={onClose}
        ...
        <div className="flex flex-col gap-2">
          {CATEGORIES.map(({ value, label, icon }) => (
            ...
            >
              <span className="w-8 h-8 rounded-full bg-blue-50 border border-blue-200 flex items-center justify-center text-xs font-bold text-blue-700">
                {icon}
              </span>
              <span>{label}</span>
            </button>
